...
app.delete('/api/review-delete', async (req, res) => {
    ...
    if (!reviewId || !bookIsbn || isNaN(deletedRating)) {
        ...
        if (!bookIsbn) missing.push('bookIsbn');
        if (isNaN(deletedRating)) missing.push('deletedRating');
        
        return res.status(400).json({ 
            error: `필수 삭제 정보가 누락되었습니다: ${missing.join(', ')}` 
        });
    }

    try {
        // 3. Firestore 참조 생성
        // (db는 server.js 상단에 const db = getFirestore(); 로 정의되어 있어야 함)
        const bookRef = db.collection('books').doc(bookIsbn);
        const reviewRef = db.collection('reviews').doc(reviewId);
        
        // 4. 트랜잭션을 사용하여 통계와 리뷰를 동시에 처리
        await db.runTransaction(async (transaction) => {
            // 4-1. 현재 책 통계 가져오기
            const bookDoc = await transaction.get(bookRef);
            if (!bookDoc.exists()) {
                console.warn(`(삭제 경고) ID ${bookIsbn} 책 통계 문서를 찾을 수 없습니다.`);
                // 책 통계가 없어도 리뷰는 삭제되도록 계속 진행
            } else {
                // 4-2. 통계가 있으면 업데이트
                const firestoreData = bookDoc.data();
                const currentReviews = firestoreData.reviews || 0;
