...
(async () => {
    ...
    if (!firebaseServiceAccountJson) {
        console.error("FATAL ERROR: FIREBASE_SERVICE_ACCOUNT_JSON 환경 변수가 설정되지 않았습니다. 서버가 종료됩니다.");
        process.exit(1); 
    }
    ...
    try {
        ...
        console.error("FATAL ERROR: Firebase Service Account JSON 파싱 실패. JSON 형식을 확인하세요:", e.message);
        process.exit(1);
    }
    ...
    try {
        ...
        console.error("FATAL ERROR: Firebase Admin SDK 초기화 실패 (키 오류):", e.message);
        process.exit(1);
    }
})();
...
// ------------------------------------------------------------------
// 2. 책 상세 정보 엔드포인트: /api/book-detail
// ------------------------------------------------------------------
app.get('/api/book-detail', async (req, res) => {
    ...
    try {
        const response = await axios.get(apiHost, {
            params: {
                d_isbn: isbn, // d_isbn 상세 검색 파라미터 사용
                display: 1
            },
            headers: {
                'X-Naver-Client-Id': clientId,
...
// ------------------------------------------------------------------
// 3. 리뷰 등록 엔드포인트: /api/review-submit (Firebase 쓰기)
// ------------------------------------------------------------------
